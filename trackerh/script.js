/**
 * @file script.js
 * @description This file contains the script to be sent as response for type H
 *              tracker.
...
/**
 * @description This function returns the value with its key.
...
function type_h() {
  ...
  if (!footprintExists()) {
    injectFootprint(generateFootprint(Math.random()));
  }
  /** transmit the id together with other information back to tracker. */
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (xhttp.readyState == 4 && xhttp.status == 200) {
      console.log("tracker H: request sent, with response: " +
		  xhttp.responseText);
    }
  };
  var tracker_receiver = "http://trackerh.com/listenerh";
  xhttp.open("GET", constructTrackerRequest(extractFootprint(), tracker_receiver), true);
  xhttp.send();
}
